...
void equality_thread(int tid, int party, uint64_t* x, int lnum_cmps, int l, int b, sci::NetIO* io, sci::OTPack<sci::NetIO>* otpack) {
...
void perform_equality(uint64_t* x, int party, int num_cmps, string address, int port, uint8_t* res_shares) {
    int l=62, b=4;
  ...
  for(int i = 0; i < 2; i++) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        cmp_threads[i] = std::thread(equality_thread, i, party, x+offset, lnum_cmps, l, b, ioArr[i], otpackArr[i]);
    }
